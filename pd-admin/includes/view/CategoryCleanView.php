<?php
    function CategoryCleanView($pdo) {
        $adjacents = 2;
        ...
        if(!isset($_POST['categorySil'])) {
            ...
            foreach ($STMRecords as $categoryView) {
                ...
                echo '<p><input type="checkbox" name="categoryClean[]" value="'.$categoryView['CID'].'" /></p>';
                echo '
                    <p>'.mb_substr($categoryView['category_name'], 0, 50, 'UTF-8').'</p>
                    <p>';
                if(($categoryView['PID'] == $categoryView['page_id']) || ($categoryView['SubPID'] == $categoryView['subpage_id'])) {
            ...
            echo '<div class="CategoryMainCenter-Line"><button onClick="return confirmDelete();" name="categorySil"><i class="fa fa-times-circle"></i>SİL</button></div>';
            echo '</form>';
        } else { categoryClean($pdo); }
        echo '</div>';
