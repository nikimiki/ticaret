<?php
    function SubCategoryCleanView($pdo) {
        $adjacents = 2;
        ...
        if(!isset($_POST['subcategorySil'])) {
            ...
            foreach ($STMRecords as $subCategoryView) {
                ...
                echo '<p><input type="checkbox" name="subcategoryClean[]" value="'.$subCategoryView['SubCID'].'" /></p>';
                echo '
            ...
            echo '<div class="SubCategoryMainCenter-Line"><button onClick="return confirmDelete();" name="subcategorySil"><i class="fa fa-times-circle"></i>SİL</button></div>';
            echo '</form>';
        } else { subcategoryClean($pdo); }
        echo '</div>';
