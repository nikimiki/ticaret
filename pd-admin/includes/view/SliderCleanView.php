<?php
    function SliderCleanView($pdo) {
        $adjacents = 2;
        $STM = $pdo->prepare("SELECT id FROM slider ");
        $STM->execute();
        ...
        slider.id as SID, slider.slider_name, slider.slider_user, slider.slider_date,
        ...
        if($lastpage > 1) {
            //$pagination .= "<div class='SliderMainBottom'>";
        ...
        echo '<div class="SliderMainCenter">';
        if(!isset($_POST['sliderSil'])) {
            foreach ($STMRecords as $sliderView) {
                echo '<form action="#" method="post">';
                echo '<div class="SliderMainCenter-Line">';
                echo '<p><input type="checkbox" name="sliderClean[]" value="'.$sliderView['SID'].'" /></p>';
                echo '<p>'.mb_substr($sliderView['slider_name'], 0, 80, 'UTF-8').'</p>
                          <p>'.$sliderView['slider_date'].'</p>
                          <p>'.$sliderView['user_name'].'</p>';
                echo '</div>';
            }
            echo '<div class="SliderMainCenter-Line"><button onClick="return confirmDelete();" name="sliderSil"><i class="fa fa-times-circle"></i>SİL</button></div>';
            echo '</form>';
        } else { sliderClean($pdo); }
        echo '</div>';
        echo '<div class="SliderMainBottom">'.$pagination.'</div>';
    }
?>