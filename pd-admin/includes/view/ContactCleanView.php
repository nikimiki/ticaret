    ...
    function ContactCleanView($pdo) {
        ...
        if(!isset($_POST['contactSil'])) {
            foreach ($DataRecords as $contactView) {
                ...
                echo '<p><input type="checkbox" name="contactClean[]" value="'.$contactView['CID'].'" /></p>';
                echo '<p>'.$contactView['phone'].'</p>
                      <p>'.$contactView['contact_date'].'</p>
                      <p>'.$contactView['user_name'].'</p>';
                echo '</div>';
            }
            echo '<div class="ContactMainCenter-Line"><button onClick="return confirmDelete();" name="contactSil"><i class="fa fa-times-circle"></i>SİL</button></div>';
            echo '</form>';
        } else { contactClean($pdo); }
        echo '</div>';
