    ...
    function ArticleCleanView($pdo) {
        ...
        if (!isset($_POST['articleSil'])) {
            ...
            foreach ($STMRecords as $articleView) {
                ...
                echo '<p><input type="checkbox" name="articleClean[]" value="'.$articleView['AID'].'" /></p>';
                ...
                if(($articleView['page_id'] == $articleView['PID']) || ($articleView['category_id'] == $articleView['CID']) || ($articleView['subcategory_id'] == $articleView['SubCID']) || ($articleView['subpage_id'] == $articleView['SubPID'])) {
                    if($articleView['page_id'] == $articleView['PID']) { echo mb_substr($articleView['page_name'], 0, 20); }
                ...
                <p style="color: #C00;">' . $articleView['user_name'] . '</p>  ';
                echo '</div>';
            }

            echo '<div class="ArticleMainCenter-Line"><button onClick="return confirmDelete();" name="articleSil"><i class="fa fa-times-circle"></i>SİL</button></div>';
            echo '</form>';
        } else { articleClean($pdo); }
        echo '</div>';
