<?php
    function PictureCleanView($pdo) {
        ...
        $STM2 = $pdo->prepare("SELECT * FROM picture ORDER BY picture.id DESC LIMIT $start, $limit");
        $STM2->execute();
        ...
        if(!isset($_POST['pictureSil'])) {
            ...
            foreach ($STMRecords as $pictureView) {
                echo '<div class="ImagesTab" style="height: 160px;margin:5px 15px;">';
                echo '
                    <img src="pic/'.$pictureView['picture'].'" alt="'.$pictureView['picture_name'].'" />
                    <input type="checkbox" name="pictureClean[]" value="'.$pictureView['id'].'">
                ';
                echo '<p style="width: 120px;">'.mb_substr($pictureView['picture_name'], 0, 24, 'UTF-8').'</p>';
                echo '</div>';
            }
            echo '<div class="PictureMainCenter-Line"><button onClick="return confirmDelete();" name="pictureSil"><i class="fa fa-times-circle"></i>SİL</button></div>';
            echo '</form>';
        } else { pictureClean($pdo); }
        echo '</div>';
